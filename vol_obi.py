...
Port of lighter_MM/vol_obi.py (itself a port of the Rust reference in
example_rust/src/strategy/obi.rs and rolling.rs).  All scaling (dollars,
ticks, vol_scale) matches the original implementation exactly.

Hot-path notes: every method is O(1) float math with no allocations
beyond the pre-sized ring buffers, so it is safe to call from the
websocket callbacks of the asyncio event loop.
"""
...
class VolObiCalculator:
    ...
    Feed it one ``on_sample(mid_price, imbalance)`` per fixed-cadence book
    update (the caller is expected to maintain incremental band totals so
    imbalance arrives precomputed), then call ``quote`` from the trading
    loop to obtain bid/ask prices.

    Scaling matches the Rust/lighter implementation exactly:
    ...
    def __init__(
        ...
        *,
        ...
        # vol_scale: converts per-message std to per-second
    ...
    def quote(self, mid_price: float, position_size: float):
        ...
        ``(None, None)`` if not warmed up or the quotes would cross.
        """
        if not self._warmed_up:
            return None, None
    ...
    @property
    def c1(self) -> float:
        """OBI alpha → fair-price shift coefficient in dollars per sigma."""
        return self._c1
